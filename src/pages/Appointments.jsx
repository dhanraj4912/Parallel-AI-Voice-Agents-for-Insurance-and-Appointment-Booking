  ...
  return (
    <div>
      ...
      <div className="table-wrapper">
        <table>
          <thead><tr><th>ID</th><th>Patient</th><th>Doctor</th><th>Date</th><th>Time</th><th>Reason</th><th>Insurance</th><th>Status</th><th></th></tr></thead>
          <tbody>
            {appointments.length === 0 && <tr><td colSpan="9"><div className="empty-state"><p>No appointments booked yet.</p></div></td></tr>}
            {appointments.map(a => (
              <tr key={a.id}>
                ...
                <td>{a.status === 'confirmed' && <button className="btn btn-outline btn-sm" onClick={() => cancel(a.id)}>Cancel</button>}</td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          <div className="modal">
            ...
            <form onSubmit={save}>
              ...
              <div className="form-group">
                ...
                <select className="form-select" required value={form.doctor_id} onChange={e => setForm({...form,doctor_id:e.target.value,appointment_time:''})}>
              ...
              <div className="form-grid-2">
                <div className="form-group">
                  ...
                  <input type="date" className="form-input" required value={form.appointment_date} min={new Date().toISOString().split('T')[0]} onChange={e => setForm({...form,appointment_date:e.target.value,appointment_time:''})} />
                </div>
                <div className="form-group">
                  <label className="form-label">Time Slot *</label>
                  <select className="form-select" required value={form.appointment_time} onChange={e => setForm({...form,appointment_time:e.target.value})}>
                    <option value="">Select slot</option>
                    {slots.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                  {form.doctor_id && form.appointment_date && slots.length === 0 && <div style={{fontSize:'0.78rem',color:'var(--color-error)',marginTop:4}}>No slots available</div>}
                </div>
              </div>
              <div className="form-group">
                <label className="form-label">Reason</label>
                <input className="form-input" placeholder="e.g. Follow-up, General checkup" value={form.reason} onChange={e => setForm({...form,reason:e.target.value})} />
              </div>
              <div className="form-group">
                <label className="form-label">Insurance Policy (optional)</label>
                <input className="form-input" placeholder="e.g. HDFC-POL-123456" value={form.insurance_used} onChange={e => setForm({...form,insurance_used:e.target.value})} />
              </div>
