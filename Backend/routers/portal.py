...
from schemas.appointment import AppointmentCreate, AppointmentOut, SlotRequest
import random
from datetime import date

router = APIRouter()

def gen_id(): return f"APT-{random.randint(100000,999999)}"
...
async def available_slots(doctor_id: int, appointment_date: str, db: AsyncSession = Depends(get_db)):
    ...
    if day not in (doctor.available_days or []):
        return {"slots": [], "message": f"Not available on {day}"}
    ...
    return {"slots": [s for s in (doctor.available_slots or []) if s not in booked_set]}
...
async def add_insurance_portal(data: dict, db: AsyncSession = Depends(get_db)):
    from models.insurance_model import InsuranceRecord
    ins = InsuranceRecord(
